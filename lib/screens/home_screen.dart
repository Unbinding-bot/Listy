...
// - NEW: Integrated ListPreviewBlock for rich, height-constrained, and faded list previews.
...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
 ...
 void _applyListsDiffs(List<Map<String, dynamic>> newLists) {
  ...
  if (!_listsLoaded) {
   ...
   if (_lists.isNotEmpty) {
    // Critical Fix: Manually insert initial items into the AnimatedList state
          ...
          Expanded(
           child: Column(
            ...
            children: [
             ...
             // 🔑 NEW: Use ListPreviewBlock for rich, formatted preview
             FutureBuilder<List<Map<String, dynamic>>>(
              future: dbService.getListItemsPreview(listId),
              builder: (context, itemSnapshot) {
               if (!itemSnapshot.hasData) return const SizedBox(height: 50, child: Center(child: Text('Loading preview...')));
               
               return ListPreviewBlock(
                items: itemSnapshot.data!,
                maxHeight: 120.0, // Set your desired max height for the preview
               );
              },
             ),
            ],
 ...
 Widget _buildListTileAnimated(Map<String, dynamic> listRow, Animation<double> animation, {bool removing = false}) {
  return SizeTransition(
   ...
   axisAlignment: 0.0,
   // Pass isReorderable=false since this is the Animated List
   child: FadeTransition(opacity: animation, child: _buildListCardContent(listRow, index: _lists.indexOf(listRow), isReorderable: false)),
  );
 }
...
// --- ListPreviewBlock is now correctly defined outside the State class ---

class ListPreviewBlock extends StatelessWidget {
 final List<Map<String, dynamic>> items;
 final double maxHeight; // Max height for the preview area
 ...
 Widget build(BuildContext context) {
  ...
  // Build the rich text content line by line
  final children = items.map((item) {
   ...
   return Row(
    ...
    children: [
     ...
     Expanded(
      child: Padding(
       padding: const EdgeInsets.only(top: 4.0, bottom: 4.0), // Added bottom padding to space lines
       child: Text(
        title,
        style: TextStyle(
         fontSize: 14,
        ...
        maxLines: 100, // Allow full wrapping within the container
  ...
  return SizedBox(
   height: maxHeight, // Constrain the height
   child: ShaderMask(
    ...
    child: SingleChildScrollView(
     physics: const NeverScrollableScrollPhysics(), // Don't allow scrolling inside the preview
     child: Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: children,
     ),
