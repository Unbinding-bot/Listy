...
// - Added ScrollController, FloatingActionButton, and logic to scroll to the bottom
//   and focus the input field when adding a new item.
// - Tightened _addNewItem logic for empty submissions.
...
class _ListDetailScreenState extends State<ListDetailScreen> with TickerProviderStateMixin {
 ...
 Future<void> _addNewItem(String title) async {
  ...
  if (trimmedTitle.isEmpty) {
   ...
   _addItemFocusNode.unfocus(); // Close keyboard on empty submission
   return;
  }
  try {
   await dbService.addItem(int.parse(widget.listId), trimmedTitle);
   _addItemController.clear();
   _addItemFocusNode.unfocus(); // Unfocus after successful submission
  } catch (e) {
   if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to add item: $e')));
  }
 }
 
 // New: Function to initiate new item creation via FAB (Scrolls and focuses)
 void _startNewItemCreation() {
  _addItemController.clear();
  _addItemFocusNode.requestFocus();
  
  // Scroll to the bottom to ensure the field is visible (above keyboard)
    ...
    child: Row(children: [
     ...
     Expanded(
      child: TextField(
       ...
       decoration: InputDecoration(hintText: 'New item...', suffixIcon: IconButton(icon: const Icon(Icons.send), onPressed: () => _addNewItem(_addItemController.text)), border: const OutlineInputBorder()),
       onSubmitted: (v) => _addNewItem(v),
      ),
     ),

     const SizedBox(width: 8),
     // Replaced original more_vert with padding space to balance the endDocked FAB
 ...
 Widget build(BuildContext context) {
  return Scaffold(
   ...
   floatingActionButton: isSelecting ? null : FloatingActionButton(
        onPressed: _startNewItemCreation,
